...
These are almost only available for the Windows operating system, and are distributed as compiled dynamic-link libraries (DLLs), one per manufacturer; the source code is generally not available.
...
The Neo object model (version 0.3) consists of 14 classes, which may be divided into three types: data objects, container objects and grouping objects.
...
\begin{itemize}
\item \lstinline`AnalogSignal`: a sampling\thomas{How about ``record'' instead of ``sampling''?}\samuel{I also prefer record} of a continuous, analog signal with a fixed sampling interval.
...
A \lstinline`Segment` may contain any of the data object types above provided all the objects share a common time basis, i.e. they all represent data recorded at the same time without breaks in the recording\thomas{``without breaks ...'' might be misunderstood. Remove?}\samuel{Agree with thomas I prefer : they all represent data recorded simultaneously}. 
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{figures/neo_graph_edit_lowres.png}
