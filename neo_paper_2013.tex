...
In neuroscience, electrophysiological signals are acquired, analysed and visualised using a large number of different software tools, but exchanging data between them is difficult, due to incompatible data models and file formats.
...
As such a common representation, we propose a language independent, hierarchical object model suitable for the representation of electrophysiological data acquired from electroencephalographic, intracellular or extracellular recordings, or generated from simulations. 
As a concrete instantiation of this object model, we have developed an open source implementation in the Python programming language, called Neo. 
In addition to representing electrophysiology data in memory for the purposes of analysis and visualisation, the Neo package provides a set of input/output (IO) modules for reading/writing the data from/to a variety of commonly used file formats, including those produced by most of the major manufacturers of electrophysiology recording equipment and more generic formats such as the Matlab file format.
...
Software for neurophysiology data analysis and visualization built on top of Neo automatically gains the benefits of interoperability, easier data sharing and automatic format conversion: there is already a burgeoning ecosystem of such tools. 
We intend that Neo should become the standard base package for Python tools in neurophysiology. The object model is equally suitable for other object-oriented languages used in neuroscience, such as Java or Matlab.
...
An advantage of this compared to the other two approaches mentioned above is that it encourages modularity in software development: the data-conversion library defines certain data structures which can then be used by developers of analysis and visualization tools, which thereby gain greater interoperability.
...
For example, in the case of \lstinline`AnalogSignal`, the required attributes are the sampling interval (or, equivalently, the sampling rate), the timestamp of the first sample and the units of the signal, for example ``millivolts'' in the case of the membrane potential. 
...
A number of projects and laboratories are already using Neo. A total of five different laboratories are developing and using daily tools based on Neo. A brief overview of these projects is helpful to gauge the benefit.
...
The purpose of the platform is to provide a centralized system for organizing and sharing of electrophysiology data.
...
The G-Node platform complements the Neo data representation with metadata storage using an unrestrictive format \citep{Grewe2011}, enabling extensive data annotation in a standardized way.
...
Spyke Viewer \citep{Proepper2013} is a flexible and extensible graphical application for navigating, analyzing and visualizing electrophysiology data. 
...
\missing{Add a comment on interoperability, e.g. data generated by PyNN, stored in G-Node, analyzed/visualized in Spyke Viewer and OpenElectrophy.}
...
The Neo object model was designed after discussion between a group of scientists and engineers, both developers and potential users, from several laboratories. However, even though this model was designed carefully, it is far from perfect. The current model is the outcome of an evolution, Neo is now version 0.3. So the main compromise is to balance between rapid change in Neo to fit all new needs during building new software and slow changes to ensure stability for other software already built. New improvements are already in discussion for a future Neo 0.4. Even if it is a general issue in software developpement, this has to be taken into account even more carefully because Neo is the base of several projects. 

Some improvement are planned for future versions. (i) The memory management for IO must be improved. Despite the lazy/cascade mechanism most Neo IO classes do not allow loading only a chunk or a subset of data. (ii) The model will be simplified; in the current model some objects are redundant due to duplication between scalar representation and array representation (\lstinline`AnalogSignal` and \lstinline`AnalogsignalArray`). (iii) Some relationships are not efficient for querying (filtering) an obect when the dataset is large. For instance, looping over \lstinline`Unit`s inside a \lstinline`Block` is not straigthforward.

The development of Neo has so far focused on the representation of recorded data, as typically stored in data files during an electophysiology experiment.
...
Such developments should be coordinated with international standardization efforts, such as those of the INCF Data Sharing Program \citep{Teeters2013}.
