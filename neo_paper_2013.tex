...
$^{5}$G-Node, Department Biology II, Ludwig-Maximilians-Universität M\"unchen, 82152 Planegg-Martinsried, Germany \\
...
Our long-term goal is for Neo to become the standard base package for Python tools in neurophysiology.
...
In systems neuroscience and computational neuroscience, as in many areas of science, a considerable amount of researchers' time is spent on analysing and visualising data, using some kind of software.
...
It was decided that the object model should contain only classes for representing data and for reading/writing data from/to files or databases. Further, the behaviour of these objects should allow only basic manipulations such as rearranging datasets or extracting sub-sets. In particular, we wished to specifically exclude visualisation or analysis methods, even such simple ones as taking the mean. The motivation for this was to keep Neo as minimal as possible. In our experience developing open-source software, small, focused libraries are much more likely to see widespread uptake.
...
A \lstinline`Segment` may contain any of the data object types provided all the objects share a common time basis, i.e. they represent data recorded simultaneously. 
...
\item \lstinline`Unit`: a \lstinline`Unit` gathers all the \lstinline`Spike` and \lstinline`SpikeTrain` objects within a common \lstinline`Block`, possibly across several \lstinline`Segments`, that have been emitted by the same cell. A \lstinline`Unit` is linked to the \lstinline`RecordingChannelGroup` objects from which it was detected.
\end{itemize}

Another way to think of this organisation is that container objects provide a grouping across time while grouping objects provide a grouping across space.
...
\caption{The Neo object model: the principal classes and their relationships. For each class, an horizontal line separates the required attributes from the suggested attributes. The stars mark the attributes that have an associated unit.}\label{fig:relationships}
\end{figure}
...
The universality of the Neo object model for the representation of electrophysiolgical data coupled with the availability of a large set of IO modules for common file formats in the Python package makes Neo a very convenient and powerful base package to build upon when developing software to manipulate, analyse or display electrophysiological data in Python.
